...
function describeLoginError(error: any): string {
  if (error?.code === 'ERR_NETWORK' || error?.message === 'Network Error') {
    return 'No se pudo conectar con el servidor. Verifica que el backend esté corriendo en http://localhost:8000.';
  }
  ...
  if (status === 401) return 'Credenciales inválidas. Revisa tu correo y contraseña.';
  if (status === 400) return 'Datos incompletos. Revisa el formulario.';
  if (status === 403) return 'Tu cuenta está bloqueada o no tiene permisos.';
  if (status === 500) return 'Error interno del servidor. Inténtalo de nuevo en un momento.';
