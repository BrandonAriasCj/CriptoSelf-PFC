...
const PriceChart: React.FC<PriceChartProps> = ({ 
  ...
  // Solo dibujar/considerar posiciones abiertas del par seleccionado.
  // Las cerradas se mantienen en el array padre para el historial, pero
  // no deben dejar líneas/SL/TP/marcadores residuales en la gráfica.
  const openPositionsForPair = React.useMemo(
    () => positions.filter(p => p.status === 'open' && p.pair === selectedPair),
  ...
  const drawChart = () => {
    ...
    // Dibujar posiciones de trading (solo abiertas del par actual)
    if (openPositionsForPair.length > 0) {
      openPositionsForPair.forEach((position: Position) => {
  ...
  useEffect(() => {
    drawChart();
  }, [priceData, currentPrice, openPositionsForPair, externalCurrentPrice, showSMA, showEMA, smaPeriod, emaPeriod]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      // Verificar si el mouse está sobre alguna posición (solo abiertas del par)
  ...
  }, [openPositionsForPair, priceData, selectedPair]);
  ...
  return (
    <div className="relative w-full h-[400px] bg-card">
        ...
        <div className="bg-background/90 backdrop-blur-sm rounded-lg border min-w-[160px] shadow-sm">
          ...
          {!isPriceInfoCollapsed && (
            <div className="px-3 pb-3">
              ...
              {openPositionsForPair.length > 0 && (
                <div className="text-xs text-blue-600 mt-1">
                  {openPositionsForPair.length} posición(es) activa(s)
                </div>
              )}
            </div>
          )}
      ...
      <div className="flex flex-col gap-2 min-w-[160px]">
        ...
        {/* Leyenda de operaciones (solo si hay posiciones abiertas del par) */}
        {openPositionsForPair.length > 0 && (
          <div className="bg-background/90 backdrop-blur-sm rounded-lg border">
